...
INSERT INTO HOBBIES (Hobby) SELECT SUBSTRING_INDEX((SUBSTRING_INDEX(Hobbies,', ',1)),', ',-1) AS Hobby FROM UNF
UNION SELECT SUBSTRING_INDEX((SUBSTRING_INDEX(Hobbies,', ',2)),', ',-1) AS Hobby FROM UNF
UNION SELECT SUBSTRING_INDEX((SUBSTRING_INDEX(Hobbies,', ',3)),', ',-1) AS Hobby from UNF;

DROP TABLE IF EXISTS StudentHobbies;

CREATE TABLE StudentHobbies AS SELECT Id AS StudentId, HobbyId FROM UNF JOIN HOBBIES2 ON UNF.Hobbies = HOBBIES2.Hobby;
